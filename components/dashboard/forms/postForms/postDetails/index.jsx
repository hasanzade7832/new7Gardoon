...
const MidBannerDetails = ({ goalId }) => {
   ...
   useEffect(() => {
      const postsUrl =
         "https://7gardoon-servers.liara.run/api/posts-rel";
      axios
         .get(postsUrl)
         .then((d) => {
            setposts(d.data);
   ...
   const updater = (e) => {
      ...
      const url = `https://7gardoon-servers.liara.run/api/update-post/${goalId}`;
      axios
         .post(url, formData)
         .then((d) => {
   ...
   const remover = () => {
      const url = `https://7gardoon-servers.liara.run/api/delete-post/${goalId}`;
      axios
         .post(url)
         .then((d) => {
   ...
   useEffect(() => {
      axios
         .get(
            `https://7gardoon-servers.liara.run/api/get-post-by-id/${goalId}`
         )
         .then((d) => {
            setfullData(d.data);
            setTag(d.data.tags);
